...
// written by `DrizzleSessionStorageSQLite` at runtime, not seeded (ADR 0012:
// "session storage is the only persisted table — no example app table").
//
// `db:seed` stays in the bootstrap chain (`db:up` = mkdir .data -> db:migrate ->
...
async function seed(): Promise<void> {
  void db;
  console.log('db:seed — no seed data (session storage is the only persisted table — ADR 0012).');
}
