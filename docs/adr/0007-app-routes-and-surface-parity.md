...
| `/` | page | Non-embedded fallback. `?shop` / `host` present → `redirect({ href: `/app${search}` })` (ENG-2330, query-string preserved). Otherwise renders **plain HTML** (not `<s-page>`): "This app must be opened from your Shopify admin." Polaris web components relocate into App Bridge chrome that does not exist on this URL, so they paint blank. No login form, no redirect. |
